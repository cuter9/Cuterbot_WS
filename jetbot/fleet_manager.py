...
class Fleeter(traitlets.HasTraits):
    ...
    def stop_run(self, change):
        ...
        plot_exec_time(self.road_cruiser.execution_time[1:], self.road_cruiser.fps[1:], cruiser_model_name, self.road_cruiser.cruiser_model_str)
        
        # plot exection time of fleet controller model processing
        fleet_model_name = "fleet controller model"
        plot_exec_time(self.execution_time[1:], self.fps[1:], fleet_model_name, self.follower_model.split(".")[0])
