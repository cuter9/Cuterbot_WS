...
def load_tune_pth_model(pth_model_name="resnet18", pretrained=True):
    ...
    if torchvision.__version__.split(".")[0] >= 13:  # use "weights" in a torchvision version >= 13
        ...
        model = getattr(pth_models, pth_model_name)(num_classes=2, weights=weights)  # for fine-tuning

    else:  # use pretrained in torchvision version < 13
        model = getattr(pth_models, pth_model_name)(num_classes=2, pretrained=pretrained)
